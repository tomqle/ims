...
public interface RoleRepository extends CrudRepository<Role, Long>, PagingAndSortingRepository<Role, Long> {
	@Query(value = "SELECT r1 FROM Role r1 left join fetch r1.privileges ",
	...
	@Query(value = "SELECT r1 FROM Role r1 left join fetch r1.privileges where r1.id = ?1")
	public Optional<Role> findById(long id);
	@Query(value = "SELECT r1 FROM Role r1 left join fetch r1.privileges where lower(r1.name) = lower(?1)")
